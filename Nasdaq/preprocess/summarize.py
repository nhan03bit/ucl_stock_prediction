...
def from_csv_summarize(folder_path, saving_path):
    ...
    for csv_file in csv_files:
        ...
        # text_ratio = 0.25
        # Process text column; here is a simple example converting text column content to uppercase
        ...
        # Delete redundant rows to avoid duplication
        df = df.drop(columns=['Text'])
        # Delete rows where mark column is not 1
        df = df[df['Mark'] == 1]
        print(time.time()-a, "s")
